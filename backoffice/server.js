...
/* ACCORDIONS, TILFØJET AF MIG SELV FORDI IKKE VAR! */
app.get("/api/accordions", async (req, res) => {
  try {
    const acc = await Accordion.find();
    res.status(200).json(acc);
  } catch (error) {
    res.status(500).json({ message: "Fejl ved hentning af accordions", error });
  }
});
